"""Generate parity fixtures for PhasorLCA (PyTorch side)."""
...
CASES = [
    # (case, in_dims, d_model, n_heads, n_anchors, init_mode, L, B, scale, period, seed)
    ("lca_default_h2_A3", 4,  8, 2, 3,  "default", 12,   3, 3.0, 1.0, 51),
    ("lca_hippo_h4_A8",   8, 16, 4, 8,  "hippo",   16,   3, 3.0, 1.0, 52),
    ("lca_long_seq",      6, 12, 3, 6,  "hippo",   128,  2, 2.5, 1.0, 53),
    ("lca_2d_input",      6, 12, 3, 4,  "hippo",   None, 5, 3.0, 1.0, 54),
]
...
def main(out_dir: str | None = None) -> None:
    ...
    for case in CASES:
        name, in_d, d_model, n_heads, n_anchors, init_mode, L, B, scale, period, seed = case
        g = torch.Generator().manual_seed(seed)
        layer = PhasorLCA(
            in_d, d_model, n_heads, n_anchors,
            init_scale=scale,
            init_mode=init_mode,
            spk_args=SpikingArgs(t_period=period),
            generator=g,
        )
        if L is None:
