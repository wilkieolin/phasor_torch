...
from phasor_torch.layers.phasor_residual import PhasorTransformerBlock, PhasorResidual, _PhasorFFN
...
INTERESTING = (PhasorTransformerBlock, PhasorResidual, _PhasorFFN, PhasorLCA, PhasorDense, SSMReadout)
...
def _pf(ctx, z, threshold):
    y = _of(ctx, z, threshold)
    ctx.cur = PROBE["cur"]
    with torch.no_grad():
...
def main():
    ...
    print(f"trial={args.trial}  lr={train_cfg.lr:.3e} (x{args.lr_mult})  "
          ...
          f"blocks={model_cfg.n_blocks}/{model_cfg.block_type}  ds={ds}  device={device}", flush=True)
    ...
    def dump(tag, step, loss, ap_, pg_, probe, alph):
    ...
    while step < args.max_steps:
        ...
        bad = (not finite(lv)) or (not finite(gmax)) or (not finite(pr["max_dz"]))
        if bad:
            if prev is not None:
                dump("LAST FINITE", *prev)
            dump("BLOW-UP", step, lv, ap_, pg_, pr, al)
            # which boundary is largest, and the amplification ratio to input side
            fin_ap = [(n, v) for n, v in ap_ if finite(v)]
            if fin_ap:
                top_b = max(fin_ap, key=lambda t: t[1])
                print(f"\n  >> largest finite boundary grad: {top_b[0]} = {top_b[1]:.3e}")
        ...
        if step % args.every == 0:
            ...
            print(f"step {step:5d} loss={lv:6.3f} | gmax={gmax:.2e}@{top_b[0]:8s} "
                  f"pmax={top_p[1]:.2e}@{top_p[0]:8s} | minZ={pr['min_z']:.1e} "
                  f"maxDZ={pr['max_dz']:.1e}@{dz_src[0]:8s} | a={amean:+.3f}", flush=True)
        prev = (step, lv, ap_, pg_, pr, al)
        opt.step()
        step += 1
